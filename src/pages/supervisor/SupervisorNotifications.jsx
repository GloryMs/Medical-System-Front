...
const SupervisorNotifications = () => {
  ...
  const loadNotifications = async () => {
    try {
      const data = await execute(() => supervisorService.getNotifications(user.id));
  ...
  const loadNotificationSettings = async () => {
    try {
      const settings = await execute(() => supervisorService.getNotificationSettings());
  ...
  const markAllAsRead = async () => {
    try {
      await execute(() => supervisorService.markAllNotificationsAsRead(user.id));
      setNotifications(prev =>
  ...
  const updateNotificationSettings = async (newSettings) => {
    try {
      await execute(() => supervisorService.updateNotificationSettings(newSettings));
  ...
  const handleNotificationClick = (notification) => {
    ...
    switch (notification.type?.toLowerCase()) {
      case 'appointment':
        navigate('/app/supervisor/appointments');
        break;
      case 'case':
        if (notification.relatedId) {
          navigate(`/app/supervisor/cases/${notification.relatedId}`);
        } else {
      ...
      case 'patient':
        if (notification.relatedId) {
          navigate(`/app/supervisor/patients/${notification.relatedId}`);
        } else {
          navigate('/app/supervisor/patients');
        }
        break;
      case 'payment':
        navigate('/app/supervisor/payments');
        break;
      case 'coupon':
        navigate('/app/supervisor/coupons');
        break;
      case 'system':
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="space-y-6">
          <p className="text-gray-600">
            Customize how you receive notifications about your patients, cases, and appointments.
          </p>
          ...
          <div className="space-y-4">
            {[
              ...
              { key: 'caseUpdates', label: 'Case Updates', description: 'Receive updates on medical cases' },
              { key: 'patientUpdates', label: 'Patient Updates', description: 'Get notified about patient activities' },
